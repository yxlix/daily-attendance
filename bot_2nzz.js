...
let cookiesArr = [];
let cookie = '';
if (process.env.WWW2NZZ_COOKIE) {
    if (process.env.WWW2NZZ_COOKIE.indexOf('&') > -1) {
        cookiesArr = process.env.WWW2NZZ_COOKIE.split('&');
    } else if (process.env.WWW2NZZ_COOKIE.indexOf('\n') > -1) {
        cookiesArr = process.env.WWW2NZZ_COOKIE.split('\n');
    } else {
        cookiesArr = [process.env.WWW2NZZ_COOKIE];
    }
}
...
!(async () => {
    if (!cookiesArr[0]) {
        $.msg($.name, `【提示】请先设置环境变量 WWW2NZZ_COOKIE`,);
        return;
    }
...
function getFormhash() {
    const options = {
        "url": 'http://www.2nzz.com/index.php',
        "headers": {
            "Cookie": cookie,
